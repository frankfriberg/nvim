...
local function consolidate_windows(valid_normal_buffers)
  ...
  table.sort(wins_to_close, function(a, b)
    return a > b
  end)
...
function M.smart_buffer_close(user_opts)
  ...
  if not ignore_changes and vim.bo[current_buf].modified and #wins_displaying_current_buf <= 1 then
    local choice =
      vim.fn.confirm(string.format("Save before closing the buffer? (%s)", bufname), "&Yes\n&No\n&Cancel", 1)
    if choice == 1 then
  ...
  if not ignore_changes and buftype == "terminal" then
    if not is_terminal_idle(current_buf) then
      local choice = vim.fn.confirm(string.format("Terminal has job running. (%s)", bufname), "&Ignore\n&Cancel", 1)
      if choice == 1 then
        ignore_changes = true
      else
        return
